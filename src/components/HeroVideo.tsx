...
export const HeroVideo: React.FC<Props> = (props) => {
    ...
    useEffect(() => { // spefic ID for the video in contentful
        client.getAsset('772l5OashVngRRR1AzXL2r').then((entry) => { setVideoUrl(entry.fields.file?.url) });
    }, []);

    return <div className='overflow-hidden video_container' style={{ maxHeight: "50vh" }}>
        {videoUrl ? <video className='video_content' width="100%" height="80px" src={videoUrl} autoPlay muted loop></video> : null}
    </div>
}